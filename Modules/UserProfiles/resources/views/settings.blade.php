...
@section('content')
<form method="POST"
action="{{ route('profile.cover.update', $user->id) }}"
...
<div class="position-relative mx-n4 mt-n4">
  <div class="profile-wid-bg profile-setting-img">
    ...
    src="{{ $user->cover_photo ? asset('storage/' . $user->cover_photo) : asset('build/images/profile-bg.jpg') }}"
...
<div class="modal fade" id="chooseCoverModal" tabindex="-1" aria-labelledby="chooseCoverModalLabel" aria-hidden="true">
    ...
    <div class="card mt-n5">
            ...
            <div class="text-center">
                <div class="profile-user position-relative d-inline-block mx-auto mb-4">
                    <img id="user-avatar-img"
                         src="{{ $user->avatar ? asset('storage/' . $user->avatar) : asset('build/images/users/avatar-1.jpg') }}"
                ...
                <h5 class="fs-17 mb-1">{{ $user->first_name }} {{ $user->last_name }} </h5>
        ...
        <form id="socialsForm" method="POST" action="{{ route('profile.socials.update', $user->id) }}">
                        @csrf
                    ...
                    @foreach ($platforms as $platform)
                        <div class="text-center" style="width: 72px;">
                            <button type="button"
                                    ...
                                    onclick="addSocialInput('{{ $platform['name'] }}', '{{ $platform['icon'] }}', '{{ $platform['color'] }}'); hideChooseSocialModal();">
                                <span class="avatar-title rounded-circle d-inline-flex justify-content-center align-items-center {{ $platform['color'] }} text-white fs-4"
                                      style="width: 48px; height: 48px;">
                                    <i class="{{ $platform['icon'] }}"></i>
                                </span>
                            </button>
                            <div class="small text-muted">{{ $platform['name'] }}</div>
                        </div>
                    @endforeach
                </div>
            </div>
                ...
                <div class="card-body p-4">
                    <div class="tab-content">
                        <div class="tab-pane active" id="personalDetails" role="tabpanel">
                            ...
                            <form id="profileUpdateForm" method="POST" action="{{ route('profile.update', $user->id) }}">                                @csrf
                                <div class="row">
                            
                                    <!-- First Name -->
                                    <div class="col-lg-6">
                                        <div class="form-floating mb-3">
                                            <input type="text" name="first_name" class="form-control" id="firstnameInput" placeholder="First Name" value="{{ $user->first_name }}">
                                    ...
                                    <div class="col-lg-6">
                                        <div class="form-floating mb-3">
                                            <input type="text" name="last_name" class="form-control" id="lastnameInput" placeholder="Last Name" value="{{ $user->last_name }}">
                                    ...
                                    <div class="col-lg-6">
                                        <div class="form-floating mb-3">
                                            <input type="text" name="phone" class="form-control" id="phonenumberInput" placeholder="Phone Number" value="{{ $user->phone }}">
                                    ...
                                    <div class="col-lg-6">
                                        <div class="form-floating mb-3">
                                            <input type="email" name="email" class="form-control" id="emailInput" placeholder="Email Address" value="{{ $user->email }}">
                                    ...
                                    <div class="col-lg-12">
                                        <div class="form-floating mb-3">
                                            <textarea class="form-control" name="bio" id="bioInput" placeholder="Enter your bio" style="height: 100px">{{ $user->bio }}</textarea>
                                    ...
                                    <div class="col-lg-4">
                                        <div class="form-floating mb-3">
                                            <input type="text" name="suburb" class="form-control" id="suburbInput" placeholder="Suburb" value="{{ $user->suburb }}">
                                    ...
                                    <div class="col-lg-4">
                                        <div class="form-floating mb-3">
                                            <input type="text" name="state" class="form-control" id="stateInput" placeholder="State" value="{{ $user->state }}">
                                    ...
                                    <div class="col-lg-4">
                                        <div class="form-floating mb-3">
                                            <input type="text" name="postcode" class="form-control" id="zipcodeInput" placeholder="Postcode" value="{{ $user->postcode }}">
...
<!-- Change Password -->
<div class="col-lg-6">
    <form id="changePasswordForm" method="POST" action="{{ route('profile.password.update', $user->id) }}">
                @csrf
    ...
    <div class="d-flex justify-content-between align-items-start mb-2">
        ...
        <div class="form-check form-switch">
            <input class="form-check-input security-toggle" type="checkbox"
                   ...
                   {{ $user->secondary_verification ? 'checked' : '' }}>
        </div>
    </div>
    <hr class="mt-0 mb-2">
    <p class="text-muted pt-2 mb-0">
        ...
        <div id="existingBackupCodes" class="pt-2">
            ...
            <div class="bg-light border rounded p-3 small">
                @foreach($user->backupCodes as $code)
                    <code>{{ $code->code }}</code> {!! $code->used ? '<small class="text-danger">(used)</small>' : '' !!}<br>
                @endforeach
            </div>
        </div>
    @endif

    {{-- JS-injected codes (on fresh generation) --}}
                                ...
                                <div class="flex-shrink-0 ms-sm-3 align-self-start">
                                    <div class="form-check form-switch">
                                        <input
                                            ...
                                            type="checkbox"
                                            data-id="{{ $notif['id'] }}"
                                            {{ ($settings[$notif['id']] ?? false) ? 'checked' : '' }}
                                        />
                                    </div>
                                </div>
                            </div>
...
@section('script')
<script>
    window.validatePasswordUrl     = "{{ route('profile.password.validate') }}";
    window.defaultCoverUrl         = "{{ route('profile.cover.list') }}";
    window.chooseCoverUrl          = "{{ route('profile.cover.choose') }}";
    window.loginHistoryClearUrl    = "{{ route('profile.login-history.clear') }}";
    window.generateBackupCodeUrl = "{{ route('profile.backup.generate') }}";
    window.notificationToggleUrl = "{{ route('profile.notifications.toggle') }}";
...
<script src="{{ asset('js/modules/userprofiles/settings.js') }}"></script>
@endsection